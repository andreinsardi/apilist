...
def read_authors():
    ...
    for author in authors:
        ...
        posts_autor = session.query(Post).filter_by(authorid=author.id).all()
        posts_list_autor = []
        for post in posts_autor:
            posts_dic_autor = {'id': post.id, 'text': post.text, 'created': str(post.created), 'authorid': post.authorid, 'subtitle': post.subtitle}
            posts_list_autor.append(posts_dic_autor)
        authors_dict['posts'] = posts_list_autor
